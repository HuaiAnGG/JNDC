...
import google.architecture.coremodel.datamodel.http.service.LoginApiService;
...
public class ApiClient{
    ...
    /**带参数的retrofit单例*/
    private static Retrofit retrofitInstanceWithData;
    private static Retrofit getRetrofitInstance(String url){
        if (retrofitInstanceWithData == null) {
            synchronized (ApiClient.class) {
                if (retrofitInstanceWithData == null) {
                    retrofitInstanceWithData = new Retrofit.Builder()
                            .baseUrl(url)
                            .addConverterFactory(GsonConverterFactory.create())
                            .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                            .client(getOkHttpClientInstance())
        ...
        return retrofitInstanceWithData;
    }
